...
export function StatusBar({ novelId, focusMode, onToggleFocus }: Props) {
  ...
  return (
    <>
      <div
        className={cn(
          'h-7 border-t border-border bg-card/80 flex items-center px-3 shrink-0 text-xs',
          focusMode && 'h-6'
        )}
      >
        {!focusMode && (
          <>
            ...
            <span className="flex items-center gap-1 text-muted-foreground mx-2">
        ...
        >
          {focusMode ? (
            <><Minimize2 className="w-3 h-3" /> 退出专注</>
          ) : (
            <><Maximize2 className="w-3 h-3" /> 专注</>
          )}
        </button>
      </div>
